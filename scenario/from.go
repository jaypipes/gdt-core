...
// failed
func FromReader(r io.Reader, path string) (gdttypes.Runnable, error) {
	contents, err := ioutil.ReadAll(r)
	if err != nil {
		return nil, err
	}
	return FromBytes(contents, path)
}

// FromBytes returns a Scenario after parsing the supplied contents
func FromBytes(contents []byte, path string) (gdttypes.Runnable, error) {
